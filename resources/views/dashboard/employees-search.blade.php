...
@component('dashboard.partials.content')
	...
	@slot('content')
		...
		<table id="employees" class="emp-list">
			<thead>
  				<tr>
				    ...
				    <th rowspan="2">Join Date</br><span style="display:inline-block; font-size:13px;">(dd-mm-yyyy)</span></th>
				    <th rowspan="2">DoB</br><span style="display:inline-block; font-size:13px;">(dd-mm-yyyy)</span></th>
  				...
  				<tr>
				    <td>RP No.</td>
				    <td>Exp Date</br><span style="display:inline-block; font-size:13px;">(dd-mm-yyyy)</span></td>
				    <td>PPT No.</td>
				    <td>Exp Date</br><span style="display:inline-block; font-size:13px;">(dd-mm-yyyy)</span></td>
				    <td>Basic</td>
				    <td>Allowance</td>
		...
		<h4 class="vac-label">On Vacation</h4>
		<table id="vacation">
			<thead>
  				<tr>
				    <th rowspan="2">SN</th>
			...
			<tbody>
				...
				<tr>
					...
					<td>{{$vac->user()->first()->joined->format('d/m/Y')}}</td>
					<td>{{$vac->user()->first()->dob->format('d/m/Y')}}</td>
					<td>{{$vac->user()->first()->qid}}</td>
					<td>{{$vac->user()->first()->qid_expiry->format('d/m/Y')}}</td>
					<td>{{$vac->user()->first()->passport}}</td>
					<td>{{$vac->user()->first()->passport_expiry->format('d/m/Y')}}</td>
